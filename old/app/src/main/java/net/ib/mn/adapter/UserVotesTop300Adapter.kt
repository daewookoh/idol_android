...
import net.ib.mn.databinding.HeartVoteRankingItemWithIdolBinding
...
class UserVotesTop300Adapter(
    context: Context,
    private val mGlideRequestManager: RequestManager
) : ArrayAdapter<UserModel>(
    context, R.layout.heart_vote_ranking_item_with_idol
) {
    private val mMyRank: String? = null
    private val mAccount: IdolAccount? = null

    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        val binding: HeartVoteRankingItemWithIdolBinding

        if(convertView == null) {
            binding = HeartVoteRankingItemWithIdolBinding.inflate(
            ...
            binding = convertView.tag as HeartVoteRankingItemWithIdolBinding
        }
    ...
    override fun update(view: View?, item: UserModel, position: Int): Unit = with(view?.tag as HeartVoteRankingItemWithIdolBinding) {
        ...
        if (item.most != null) {
            myLoveIdol.text = context.getString(if (BuildConfig.CELEB) R.string.actor_most_favorite else R.string.most_favorite) +
                " : " + item.most!!.getName(context)
        } else myLoveIdol.setText(
            context.getString(if (BuildConfig.CELEB) R.string.actor_most_favorite else R.string.most_favorite) +
