...
class StatsAwardFinalTopViewHolder(
    val binding: StatsAwardsResultHeaderBinding,
    val visibleBackground: Boolean = true,
    val glideRequestManager: RequestManager,
    private val awardStatsData: AwardStatsModel?,
    ...
    fun bind(item: HallModel) {
        with(binding) {
            ...
            val title = UtilK.getAwardTitle(awardStatsData?.resultTitle, item.chartName) // item.chartName에 각 부분 이름 들어있음
            ...
            inTopAwards.root.visibility = View.VISIBLE
