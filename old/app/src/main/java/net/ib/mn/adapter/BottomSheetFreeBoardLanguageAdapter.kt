...
class BottomSheetFreeBoardLanguageAdapter(
    private val context: Context,
    private val langList: List<BoardLanguage>,
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var languageClickListener : OnClickListener? = null

    interface OnClickListener {
        fun onItemClicked(locale: String?, langId: String)
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as ViewHolder).apply {
            binding.tvSmallTalkLanguage.text = context.getString(langList[position].labelResId)
            binding.tvSmallTalkLanguage.setOnClickListener {
                val locale = when (position) {
                    0 -> {  //모든 언어
                        ...
                        langList[position].code
                    }
                }

                languageClickListener?.onItemClicked(locale, langList[position].code)
            }
        }
    }
