...
class SobaAggregatedAdapter(
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder.itemViewType == TYPE_TOP) {
            ...
            (holder as RankViewHolder).bind(mItems[position - isOffset])
        }
    }
    ...
    fun setItems(items: ArrayList<HallModel>) {
        mItems = items
    }
    ...
    inner class RankViewHolder(val binding: AggregatedHofItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: HallModel) = with(binding) {
            // 동점자 처리
            val rankValue: Int = item.rank // rank는 0부터
            ...
            val rankCount = NumberFormat.getNumberInstance(LocaleUtil.getAppLocale(itemView.context)).format(rankValue+1)
