...
class HeartPlusFreeViewModel(
    ...
    private fun showTnk(context: Context) {
        offerwall = TnkOfferwall(context)
        offerwall?.setCOPPA(false) // true -> 13세 미만 아동을 대상으로 한 서비스일 경우
        offerwall?.setUserName(IdolAccount.getAccount(context)?.userId.toString()) // 유저 식별 값 설정
        offerwall?.applicationStarted() // TNK SDK 초기화.(가능하면 onCreate에서 실행하라고는 함)
        TnkAdManager.usePointUnit(false)
        TnkAdManager.startDefaultActivity(context)
    }
