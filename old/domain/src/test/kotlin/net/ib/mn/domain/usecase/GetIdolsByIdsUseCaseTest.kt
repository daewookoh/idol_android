...
class GetIdolsByIdsUseCaseTest {

    private val idolRepository: IdolRepository = mockk()
    private lateinit var useCase: GetIdolsByIdsUseCase

    @Before
    fun setUp() {
        useCase = GetIdolsByIdsUseCase(idolRepository)
    }

    @Test
    fun `invoke returns Flow with Loading and Success with idol list`() = runTest {
        // given
        val dummyIdols = listOf(
            createDummyIdol(id = 1),
            createDummyIdol(id = 2)
        )
        coEvery { idolRepository.getIdolsByIds(listOf(1, 2)) } returns flow {
            emit(DataResource.Loading())
            emit(DataResource.success(dummyIdols))
        }

        // when
        val result = useCase(listOf(1, 2)).toList()
