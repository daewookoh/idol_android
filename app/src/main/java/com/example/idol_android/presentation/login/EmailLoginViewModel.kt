...
import com.example.idol_android.data.local.PreferencesManager
import com.example.idol_android.data.remote.interceptor.AuthInterceptor
import com.example.idol_android.domain.model.ApiResult
import com.example.idol_android.domain.usecase.SignInUseCase
import com.example.idol_android.util.Constants
import com.example.idol_android.util.CryptoUtil
import com.example.idol_android.util.DeviceUtil
...
class EmailLoginViewModel @Inject constructor(
                ...
                signInUseCase(
                    ...
                    when (result) {
                        is ApiResult.Success<*> -> {
                            val response = result.data as com.example.idol_android.data.remote.dto.SignInResponse
    ...
    private suspend fun handleSignInSuccess(
        email: String,
        password: String,
        response: com.example.idol_android.data.remote.dto.SignInResponse
    ) {
     ...
     * - 그 외: 일반 에러 메시지 표시
     */
    private fun handleSignInFailure(response: com.example.idol_android.data.remote.dto.SignInResponse) {
        setState { copy(isLoading = false) }
