...
class HeartPickDetailViewModel @Inject constructor(
    ...
    override fun handleIntent(intent: HeartPickDetailContract.Intent) {
        when (intent) {
            ...
            is HeartPickDetailContract.Intent.LoadUserHearts -> loadUserHearts()
            is HeartPickDetailContract.Intent.VoteHeartPick -> voteHeartPick(
                intent.heartPickIdolId,
                intent.idolId,
                intent.heart
            )
        }
     ...
     */
    private fun voteHeartPick(heartPickIdolId: Int, idolId: Int, heart: Long) {
        val heartPickId = currentState.heartPick?.id ?: return

        if (heart <= 0) {
            setEffect { HeartPickDetailContract.Effect.VoteError(context.getString(R.string.required_field)) }
            return
        }

        if (heart > currentState.totalHeart) {
            setEffect { HeartPickDetailContract.Effect.VoteError(context.getString(R.string.not_enough_heart)) }
            return
        }
            ...
            heartpickRepository.voteHeartPick(
                ...
                when (result) {
                    is ApiResult.Success -> {
                        ...
                        // 하트 차감
                        val newTotalHeart = currentState.totalHeart - heart
                        val newFreeHeart = if (currentState.freeHeart > 0) {
                            val usedFreeHeart = minOf(heart, currentState.freeHeart)
                            currentState.freeHeart - usedFreeHeart
                        } else {
                            currentState.freeHeart
                        }

                        // DataStore 캐시 업데이트
                        val newStrongHeart = newTotalHeart - newFreeHeart
                        preferencesManager.updateUserHearts(newStrongHeart, newFreeHeart)

                        setState {
                            copy(
                                totalHeart = newTotalHeart,
                                freeHeart = newFreeHeart,
                                isVoting = false
                            )
                        }

                        // 아이돌 투표수 UI 업데이트
                        updateIdolVote(idolId, heart.toInt())

                        setEffect {
                            HeartPickDetailContract.Effect.VoteSuccess(
                                bonusHeart = response.bonusHeart,
                    ...
                    is ApiResult.Error -> {
                        ...
                        setEffect {
                            HeartPickDetailContract.Effect.VoteError(
                                result.message ?: context.getString(R.string.desc_failed_to_connect_internet)
                            )
                        }
                    }
