...
class HeartPickDetailContract {
    ...
    data class State(
        ...
        // 투표 관련 상태
        val totalHeart: Long = 0L,         // 사용자 총 하트 (에버하트 + 데일리하트)
        val freeHeart: Long = 0L,          // 사용자 데일리하트 (무료 하트)
        val isVoting: Boolean = false      // 투표 진행 중 여부
    ) : UiState
    ...
    sealed class Intent : UiIntent {
        ...
        /** 사용자 하트 정보 로드 */
        data object LoadUserHearts : Intent()

        /** 하트픽 투표 실행 */
        data class VoteHeartPick(
            val heartPickIdolId: Int,
            val idolId: Int,
            val heart: Long
        ) : Intent()
    }
    ...
    sealed class Effect : UiEffect {
        ...
        /** 투표 성공 */
        data class VoteSuccess(val bonusHeart: Int, val voted: Long) : Effect()

        /** 투표 실패 */
        data class VoteError(val message: String) : Effect()
    }
}
