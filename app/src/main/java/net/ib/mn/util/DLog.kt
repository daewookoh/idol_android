...
// Exception을 받는 오버로드 함수들
fun logE(paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.e(TAG, buildLogMsg(paramString), throwable)
}

fun logE(tag: String, paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.e(tag, buildLogMsg(paramString), throwable)
}

fun logW(paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.w(TAG, buildLogMsg(paramString), throwable)
}

fun logW(tag: String, paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.w(tag, buildLogMsg(paramString), throwable)
}

fun logD(paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.d(TAG, buildLogMsg(paramString), throwable)
}

fun logD(tag: String, paramString: String, throwable: Throwable) {
    if (isNotDebugMode()) return
    Log.d(tag, buildLogMsg(paramString), throwable)
}
